...
public class OwnershipSynchronizer<R> {
    ...
    private static final AtomicReferenceFieldUpdater<OwnershipSynchronizer, Owner> OWNER = AtomicReferenceFieldUpdater.newUpdater(
            OwnershipSynchronizer.class, Owner.class, "owner");
    ...
    private final R protectedResource;
    ...
    private void assertIsOwnerThreadAndPreemptPrevented(Owner cur) {
        LettuceAssert.assertState(isOwnerCurrentThreadAndPreemptPrevented(cur),
                () -> "[executeInOwnerWithPreemptPrevention] unexpected: " + (cur.inEventLoop()
                        ? "preemption not prevented"
                        : "owner is not this thread"));
    }
